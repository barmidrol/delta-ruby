...
class OptimizeTest < Minitest::Test
  def test_compact
    ...
    with_table(df) do |dt|
      ...
      assert_in_delta 528, metrics["filesAdded"]["avg"]
      assert_equal 528, metrics["filesAdded"]["max"]
      assert_equal 528, metrics["filesAdded"]["min"]
      assert_equal 1, metrics["filesAdded"]["totalFiles"]
      assert_equal 528, metrics["filesAdded"]["totalSize"]
      assert_in_delta 514, metrics["filesRemoved"]["avg"]
      assert_equal 514, metrics["filesRemoved"]["max"]
      assert_equal 514, metrics["filesRemoved"]["min"]
      assert_equal 3, metrics["filesRemoved"]["totalFiles"]
      assert_equal 1542, metrics["filesRemoved"]["totalSize"]
